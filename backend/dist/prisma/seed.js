...
async function seed() {
    ...
    const admin = await upsertBy(() => prisma.usuario.findUnique({ where: { idUsuario: 1 } }), () => prisma.usuario.create({
        data: {
            nombreUsuario: 'admin',
    ...
    for (const p of provincias) {
        const provincia = await upsertBy(() => prisma.provincia.findFirst({ where: { nombreProvincia: p.nombreProvincia } }), () => prisma.provincia.create({ data: { nombreProvincia: p.nombreProvincia } }));
        for (const nombre of p.localidades) {
            await upsertBy(() => prisma.localidad.findFirst({
    ...
    for (const m of [
        ...
        await upsertBy(() => prisma.moneda.findFirst({ where: { moneda: m.moneda } }), () => prisma.moneda.create({ data: m }), () => prisma.moneda.updateMany({
            where: { moneda: m.moneda },
            data: { precio: m.precio }
        }).then(() => prisma.moneda.findFirst({ where: { moneda: m.moneda } })));
    }
    // 5. tipoCliente
    await upsertBy(() => prisma.tipoCliente.findFirst({ where: { tipoCliente: 'Consumidor Final' } }), () => prisma.tipoCliente.create({ data: { tipoCliente: 'Consumidor Final' } }));
    // 6. nivelCliente
    for (const n of [0, 5, 10]) {
        await upsertBy(() => prisma.nivelCliente.findFirst({ where: { indiceBeneficio: n } }), () => prisma.nivelCliente.create({ data: { indiceBeneficio: n } }));
    }
    // 7. tipoPago (venta)
    for (const t of [
        { tipoPago: "Efectivo" },
        ...
        await upsertBy(() => prisma.tipoPago.findFirst({
    ...
    for (const e of ['Pendiente', 'Finalizada', 'Cancelada']) {
        await upsertBy(() => prisma.estadoVenta.findFirst({ where: { nombreEstadoVenta: e } }), () => prisma.estadoVenta.create({ data: { nombreEstadoVenta: e } }));
    }
    // 9. metodoPago (compra)
    for (const m of ['Efectivo', 'Transferencia']) {
        await upsertBy(() => prisma.metodoPago.findFirst({ where: { metodoPago: m } }), () => prisma.metodoPago.create({ data: { metodoPago: m } }));
    }
    // 10. roles globales
    for (const r of ['Administrador', 'Vendedor', 'Cajero']) {
        await upsertBy(() => prisma.rol.findFirst({ where: { nombreRol: r } }), () => prisma.rol.create({ data: { nombreRol: r } }));
    }
    // 11. asignar rol admin al usuario admin
    const rolAdmin = await prisma.rol.findFirst({ where: { nombreRol: 'Administrador' } });
    if (rolAdmin) {
        await upsertBy(() => prisma.usuarioRol.findFirst({
    ...
    for (const c of clientes) {
        await upsertBy(() => prisma.cliente.findFirst({ where: { emailCliente: c.emailCliente } }), () => prisma.cliente.create({ data: c }), () => prisma.cliente.updateMany({
            where: { emailCliente: c.emailCliente },
            data: c
        }).then(() => prisma.cliente.findFirst({ where: { emailCliente: c.emailCliente } })));
    }
    // 14. proveedores
    const proveedores = [
        {
    ...
    for (const prov of proveedores) {
        await upsertBy(() => prisma.proveedor.findFirst({
            where: { CIF_NIFProveedor: prov.CIF_NIFProveedor }
        }), () => prisma.proveedor.create({ data: prov }), () => prisma.proveedor.updateMany({
            where: { CIF_NIFProveedor: prov.CIF_NIFProveedor },
            data: prov
        }).then(() => prisma.proveedor.findFirst({
            where: { CIF_NIFProveedor: prov.CIF_NIFProveedor }
        })));
    }
    ...
    for (const u of usuarios) {
        await upsertBy(() => prisma.usuario.findFirst({ where: { emailUsuario: u.emailUsuario } }), () => prisma.usuario.create({ data: u }), () => prisma.usuario.updateMany({
            where: { emailUsuario: u.emailUsuario },
            data: u
        }).then(() => prisma.usuario.findFirst({ where: { emailUsuario: u.emailUsuario } })));
    }
    ...
    if (pendiente && tipoEfectivo && tipoDebito && ars && camilo && luz) {
        ...
        await prisma.ventaActor.create({
            data: {
                ...
                idUsuario: creatorUserId,
                papel: client_1.PapelEnVenta.CREADOR,
            },
        });
