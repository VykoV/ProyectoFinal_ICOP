...
exports.getPresupuestoTicket = getPresupuestoTicket;
exports.getVentaTicket = getVentaTicket;
...
async function calcularTotales(idVenta) {
    ...
    if (!venta)
        return { importeArticulos: 0, totalFinal: 0, subtotal: 0, descuentoGeneralMonto: 0, recargoPagoMonto: 0 };
...
async function generateTicketPDF(res, type, id) {
    try {
        const venta = await prisma.venta.findUnique({
            where: { idVenta: id },
            include: {
        ...
        if (COMERCIO_DIRECCION)
            doc.fontSize(8).font('Helvetica').text(COMERCIO_DIRECCION, { align: 'center' });
        if (COMERCIO_TELEFONO)
            doc.text(`Tel: ${COMERCIO_TELEFONO}`, { align: 'center' });
        doc.moveDown(0.5);
        ...
        if (type === 'presupuesto') {
            ...
            if (venta.Usuario)
                doc.text(`Vendedor: ${venta.Usuario.nombreUsuario}`);
        }
        else {
            if (venta.Usuario)
                doc.text(`Cajero: ${venta.Usuario.nombreUsuario}`);
        }
        if (venta.Cliente) {
            doc.text(`Cliente: ${venta.Cliente.nombreCliente} ${venta.Cliente.apellidoCliente}`);
            if (venta.Cliente.cuil)
                doc.text(`CUIT/DNI: ${venta.Cliente.cuil}`);
            if (venta.Cliente.telefonoCliente)
                doc.text(`Tel: ${venta.Cliente.telefonoCliente}`);
        }
        else {
        ...
        venta.detalles.forEach(d => {
            ...
            if (descPct > 0)
                detalleStr += ` (Desc ${Number(d.descuentoItem)}%)`;
            if (recPct > 0)
                detalleStr += ` (Rec ${Number(d.recargoItem)}%)`;
