...
exports.getStockRotation = getStockRotation;
exports.getStagnantProducts = getStagnantProducts;
exports.getEstimatedMargin = getEstimatedMargin;
exports.getInactiveClients = getInactiveClients;
exports.getProviderDependency = getProviderDependency;
exports.getBudgetConversion = getBudgetConversion;
const client_1 = require("@prisma/client");
const pg_1 = require("pg");
const adapter_pg_1 = require("@prisma/adapter-pg");
const pool = new pg_1.Pool({ connectionString: process.env.DATABASE_URL });
const prisma = new client_1.PrismaClient({ adapter: new adapter_pg_1.PrismaPg(pool) });
const parseDate = (d) => (d ? new Date(String(d)) : undefined);
// 1. Rotación de Stock
async function getStockRotation(req, res) {
    try {
...
async function getStagnantProducts(req, res) {
    try {
...
// 3. Margen Estimado
async function getEstimatedMargin(req, res) {
    try {
        const products = await prisma.producto.findMany({
            select: {
                idProducto: true,
...
async function getInactiveClients(req, res) {
    try {
...
async function getProviderDependency(req, res) {
    try {
        const { from, to } = req.query;
        const startDate = parseDate(from);
        const endDate = parseDate(to);
...
async function getBudgetConversion(req, res) {
    try {
        const { from, to } = req.query;
        const startDate = parseDate(from);
        const endDate = parseDate(to);
        ...
        const estPendiente = estados.find(e => e.nombreEstadoVenta.toLowerCase().includes("pend") ||
            e.nombreEstadoVenta.toLowerCase().includes("presu"));
        // Finalizada
        const estFinal = estados.find(e => e.nombreEstadoVenta.toLowerCase().includes("final") ||
